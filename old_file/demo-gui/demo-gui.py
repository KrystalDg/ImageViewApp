...
class ImageBrowserApp:
    def __init__(self, root):
        ...
        self.toggle_fixed_rect_checkbox.pack(pady=10)

        # Thêm thuộc tính mới
        self.toggle_resize_border_enabled = False
        self.toggle_resize_border_button = ttk.Button(
            self.right_column_frame,
            text="Toggle Resize Border",
            command=self.toggle_resize_border,
            style="TButton",
        )
        self.toggle_resize_border_button.pack(pady=10)
    ...
    def display_image(self, image_path):
        ...
        self.border_height = int(
            new_height / 10
        )  # Chiều dài mặc định là 1/10 chiều dọc của ảnh
        self.red_rect_border = self.image_canvas.create_rectangle(
            0, 0, new_width, self.border_height, outline="red", width=2
        )
        self.red_rect_coords = (0, 0, new_width, self.border_height)

    def resize_border_height(self, event):
        if self.toggle_resize_border_enabled:
            y_delta = event.y - self.prev_y
            new_height = self.border_height + y_delta

            # Giữ chiều dài border màu đỏ trong phạm vi hợp lý
            new_height = max(0, min(new_height, self.image_canvas.winfo_height()))

            # Di chuyển border màu đỏ và cập nhật chiều dài
            self.image_canvas.move(self.red_rect_border, 0, y_delta)
            self.border_height = new_height
            self.red_rect_coords = (0, self.prev_y, self.image_width, self.border_height)
            ...
            # Cập nhật lại chiều dài của border màu đỏ
            self.image_canvas.coords(
                self.red_rect_border, 0, 0, self.image_width, self.border_height
            )
    ...
    def stop_drawing_rect(self, event):
        if self.drawing_rect and self.toggle_draw_inside and self.draw_enabled:
            ...
            # Di chuyển hình chữ nhật nếu checkbox không được chọn
            if not self.fixed_rect:
                y_delta = event.y - self.prev_y
                self.image_canvas.move(self.current_rect, 0, y_delta)

            # Cập nhật tọa độ chuột
            self.prev_y = event.y

            # Tự động vẽ lại hình chữ nhật nếu checkbox được chọn
            if self.fixed_rect:
                y_delta = (
                    self.image_canvas.coords(self.red_rect_border)[1]
                    - self.rect_start_y
                )
                self.image_canvas.move(self.current_rect, 0, y_delta)
    ...
    def reset_draw(self):
        ...
        if self.current_rect:
            ...
            # Reset lại tất cả các giá trị liên quan khi nhấn nút Reset Draw
            self.drawing_rect = False
            self.rect_start_x = 0
            self.rect_start_y = 0
            self.prev_y = 0

            # Cập nhật lại tọa độ khi nhấn Reset Draw
            if self.fixed_rect and self.red_rect_border:
                y_delta = (
                    self.image_canvas.coords(self.red_rect_border)[1]
                    - self.rect_start_y
                )
    ...
    def toggle_resize_border(self):
        self.toggle_resize_border_enabled = not self.toggle_resize_border_enabled

        if self.toggle_resize_border_enabled:
            # Kết nối sự kiện kéo thả cạnh trên và dưới
            self.image_canvas.tag_bind(
                self.red_rect_border, "<B1-Motion>", self.resize_border_height
            )
    ...
    def toggle_fixed_rect(self):
        ...
        # Di chuyển hình chữ nhật về vị trí ban đầu nếu checkbox không được chọn
        if not self.fixed_rect and self.current_rect:
            self.image_canvas.coords(
                self.current_rect,
                self.rect_start_x,
                self.rect_start_y,
        ...
        # Tự động vẽ lại hình chữ nhật nếu checkbox được chọn
        if self.fixed_rect and self.current_rect:
            y_delta = (
                self.image_canvas.coords(self.red_rect_border)[1] - self.rect_start_y
            )
            self.image_canvas.move(self.current_rect, 0, y_delta)
