...
from tensorflow.keras.layers import Dense, Conv2D, MaxPooling2D, Dropout, Flatten, LSTM, Reshape, BatchNormalization, Input, Conv2D, MaxPool2D, Lambda, Bidirectional, Add, Activation
from tensorflow.keras.models import Sequential
from tensorflow.keras import optimizers
from tensorflow.keras.utils import to_categorical
from tensorflow.keras.callbacks import ReduceLROnPlateau, ModelCheckpoint, LearningRateScheduler, EarlyStopping, TensorBoard
...
class CNN_Model(object):
    ...
    def train(self):
        ...
        # Model Checkpoint
        cpt_save = ModelCheckpoint('./weight2.h5', save_best_only=True, monitor='val_acc', mode='max')
...
class CRNN_Model(object):
    ...
    def build_model(self, istrain = False):            
        ...
        inputs = layers.Input(
                                shape=self.imgShape, name="image", dtype="float32"
                            )
        ...
        loss_out = Lambda(self.ctc_lambda_func, output_shape=(1,), name='ctc')([outputs, labels, input_length, label_length])

        #model to be used at training time
        self.model_CTC = Model(inputs=[inputs, labels, input_length, label_length], outputs=loss_out)
    ...
    def train(self, train_data, valid_data, epochs, batch_size):
        ...
        self.model_CTC.compile(loss={'ctc': lambda y_true, y_pred: y_pred}, optimizer = 'adam')
        ...
        self.model_CTC.fit( x=train_data, 
    ...
    def recognize_digit(self, digit):
        ...
        for i in range(len(digit)):
            predict_img.append(self.preprocess_img(digit[i], (128,32))*255.)
