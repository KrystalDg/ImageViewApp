...
class ImageViewerApp:
    ...
    def setup_controls(self, control_set):
        ...
        entry = ttk.Entry(control_frame, textvariable=coordinates_var)
    ...
    def move_border(self, y_delta):
        for control_set in self.control_sets:
            if control_set['fixed_rect']:
                # Di chuyển cả hình chữ nhật nếu nút "Fix" được bật
    ...
    def toggle_fixed_rect(self, control_set):
        control_set['fixed_rect'] = not control_set['fixed_rect']
    ...
    def start_drawing_rect(self, event):
        ...
        control_set['rect_coords'] = [rect_start_x, rect_start_y, rect_start_x, rect_start_y]
        control_set['current_rect'] = self.image_canvas.create_rectangle(
            rect_start_x, rect_start_y, rect_start_x, rect_start_y,
    ...
    def drag_rect(self, event):
        ...
        if control_set['fixed_rect']:
            ...
            temp_rect = self.image_canvas.create_rectangle(
                control_set['rect_coords'][0], control_set['rect_coords'][1],
                canvas_x, canvas_y,
                outline="blue", width=2,
                tags="temp_rect"
            )
    ...
    def stop_drawing_rect(self, event):
        ...
        if control_set['fixed_rect']:
            ...
            self.image_canvas.coords(
                control_set['current_rect'],
                real_rect_coords[0], real_rect_coords[1],
                real_rect_coords[2], real_rect_coords[3]
            )
            ...
            control_set['draw_enabled'] = False

            self.update_rectangle_position(control_set)

    def update_rectangle_position(self, control_set):
        rect_coords = control_set['rect_coords']
        control_set['coordinates_var'].set(
